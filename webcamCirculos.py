...
def image_da_webcam(img):
    ...
    if(img.all() != None):
        saida = detectaCirculos(saida, gray)   
        both_mask_hsv = aplicaMascara(img_hsv)
        ...
        for i in contornos:
            ...
            if M["m00"] != 0:
                ...
                texto = ' '
                origem = (0,50)
                cv2.putText(saida, str(texto), origem, fonte,1,cor,2,cv2.LINE_AA)

            saida = tracaReta(saida, cxV, cyV)
            saida = calculaAnguloReta(saida, cxV, cyV)
    return saida

def detectaCirculos(saida, gray):
...
def aplicaMascara(img_hsv):
    ...
    return both_mask_hsv

def tracaReta(saida, cxV, cyV):
    # Traça a reta
    cor = (0, 0, 0)
...
def calculaAnguloReta(saida, cxV, cyV):
